<template>
  <div class="grid grid-cols-10">
    <div class="col-span-10 lg:col-span-7 lg:mr-8">
      <h2 class="font-bold text-lg md:text-xl text-tm-black my-2">
        Create Post
      </h2>
      <div class="rounded-3xl bg-white p-4 sm:p-6 mt-2 sm:mt-4">
        <form @submit.prevent="createPost">
          <div>
            <select v-model="post.category" class="select w-full text-base font-normal bg-clip-padding rounded border border-tm-black text-tm-black focus:border-tm-green" required>
          ...
          <div class="my-4">
            <textarea
              ...
              "
              rows="5"
              placeholder="Say something to your townspeople..."
              required
            />
          </div>
          <div>
  ...
  methods: {
    serverResponse (error, file) {
      if (error) {
        ...
        this.post.upload_id = file.serverId
      }
    },
    async createPost () {
